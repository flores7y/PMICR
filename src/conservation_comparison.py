...
class RoundResult:
    ...
    cumulative_absolute_error: float
...
def aggregated_demand_reward_metrics(
    period,
    task_sizes,
    t,
    xi_max_value,
    ...
    return RewardMetrics(
        reward=upsilon / period,
        absolute_error=reservation_error,
...
def calculate_reward_metrics(
    ...
    if use_aggregated_demand_reward:
        return aggregated_demand_reward_metrics(
            period,
            task_sizes,
            t,
            xi_max_value,
        ...
        for offset in range(period):
            ...
            if actual_demand > psi_n_t:
                reservation_error_step += (
                    cfg.FACTOR
                    * cfg.MEMORY_TO_DEMAND_SCALE
                    * (actual_demand - psi_n_t)
                )
            else:
                reservation_error_step -= (
                    cfg.FACTOR
...
def reward_function(
    ...
    return calculate_reward_metrics(
        period,
        task_sizes,
        t,
        xi_max_value,
        zeta_value,
        use_aggregated_demand_reward,
    ).reward
...
def run_algorithm(
    ...
    for t in range(actual_rounds):
        ...
        cumulative_absolute_error += metrics.absolute_error
        average_reward = cumulative_reward / (t + 1)
        average_absolute_error = cumulative_absolute_error / (t + 1)
        round_results.append(
            RoundResult(
                ...
                average_absolute_error=float(average_absolute_error),
            )
        )
    ...
    summary = SummaryResult(
        ...
        cumulative_absolute_error=float(cumulative_absolute_error),
        average_absolute_error=float(cumulative_absolute_error / actual_rounds),
    ...
    for area in range(cfg.AREA_NUM):
        ...
        for algorithm in build_algorithms(cfg.ARMS, cfg.UCB_C, cfg.MAB_EPSILON):
            ...
            print(
                ...
                f"average_absolute_error={summary.average_absolute_error:.6f}, "
