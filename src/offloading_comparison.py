...
def build_scenario(sample_index=0, task=10.0, local_capacity=3.0):
...
class OffloadingSolverCore:
    ...
    def __init__(self, scenario):
        ...
        self.price_m = 0.11
        self.price_m_step = 0.21
        self.price_n = 0.10
        self.price_n_step = 0.20
...
class PMICRSolver(OffloadingSolverCore):
    ...
    def offloading_cal(self, price_m, price_n):
        ...
        offloading = (
            (
                0.6
                * (1 / self.rate_m + 10 * self.cal_per / self.f1)
                / denominator
                + self.cal_per * self.task / self.f
                + 1
            )
            / (self.task * (10 * self.cal_per / self.f1 + 1 / self.rate_m))
        )
        lower_bound = 0.22 + random.uniform(0.05, 0.1)
    ...
    def cal_pm(self):
        self.update_delays()
        self.price_m_step -= 0.05
        self.u_m_max = self.cal_u_m()
        for _ in range(40):
            self.price_m_step = self.price_m_step + (5 - self.price_m_step) / 5 * self.step
            upper = self.energy * self.f1 + 2 / self.price_factor
            if self.price_m_step > upper:
                self.price_m_step = upper
            offloading = self.offloading_cal(self.price_m_step, self.price_n)
            if abs(self.offloading - offloading) > 0.1:
                self.offloading += 0.1 if self.offloading < offloading else -0.1
            else:
                self.offloading = offloading
            self.offloading = min(max(self.offloading, 0), 0.9)
    ...
    def cal_pn(self):
        self.update_delays()
        self.price_n_step -= 0.05
        self.u_n_max = self.cal_u_n()
        for _ in range(40):
            self.price_n_step = (
                self.price_n_step + (5 - self.price_n_step) / 5 * self.step * 0.85
            )
            upper = self.energy * self.fe1 + 2 / self.price_factor
...
class WithoutRestrainSolver(PMICRSolver):
    ...
    def cal_pn(self):
        ...
        for _ in range(40):
            self.price_n_step = (
                self.price_n_step + (5 - self.price_n_step) / 5 * self.step * 0.85 * 1.2
            )
            upper = self.energy * self.fe1 + 2 / self.price_factor
            if self.price_n_step > upper:
                self.price_n_step = upper
...
class SimulatedAnnealingSolver(OffloadingSolverCore):
    ...
    def __init__(self, scenario, initial_temp=10.0, cooling_rate=0.9, stop_temp=1e-8):
    ...
    def cal_pm(self):
        ...
        for _ in range(40):
            candidate = max(0.11, min(self.price_m + random.uniform(-0.05, 0.05), 2.65))
            upper = self.energy * self.f1 * self.offloading + 2 / self.price_factor
    ...
    def cal_pn(self):
        ...
        for _ in range(40):
            candidate = max(0.10, min(self.price_n + random.uniform(-0.05, 0.05), 2.64))
            if candidate > self.price_m:
                candidate = self.price_n
            upper = self.energy * self.fe1 * self.offloading + 2 / self.price_factor
            candidate = min(candidate, upper)
    ...
    def accept(self, old_utility, new_utility):
        if new_utility > old_utility:
            return True
        probability = math.exp((new_utility - old_utility) / max(self.temp, EPS))
...
class LBFGSBSolver(OffloadingSolverCore):
    ...
    def __init__(self, scenario, max_price_sv=3.0, max_price_es=2.9):
        super().__init__(scenario)
    ...
    def run(self):
        initial_prices = [
            random.uniform(0.11, self.max_price_sv),
            random.uniform(0.10, self.max_price_es),
        ]
        if minimize is None:
        ...
        result = minimize(
            self.utility,
            initial_prices,
            bounds=[(0, self.max_price_sv), (0, self.max_price_es)],
            method="L-BFGS-B",
    ...
    def local_box_search(self, initial_prices):
        ...
        best[0] = np.clip(best[0], 0, self.max_price_sv)
        best[1] = np.clip(best[1], 0, self.max_price_es)
        best_value = self.utility(best)
        step_sizes = np.array([0.25, 0.25], dtype=float)

        while np.max(step_sizes) > 1e-4:
            improved = False
            for dim in range(2):
                for direction in (-1, 1):
                    candidate = best.copy()
                    candidate[dim] += direction * step_sizes[dim]
                    candidate[0] = np.clip(candidate[0], 0, self.max_price_sv)
                    candidate[1] = np.clip(candidate[1], 0, self.max_price_es)
...
def main():
    ...
    parser.add_argument("--samples", type=int, default=1, help="Number of sample indices to run.")
    parser.add_argument(
        "--output",
        default="offloading_comparison_results.csv",
